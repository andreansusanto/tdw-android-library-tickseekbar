...
public class TickSeekBar extends View {
    ...
    private void initParams() {
        ...
        int textLine = getMaxTextLine();
        mCustomDrawableMaxHeight = Math.max(mThumbTouchRadius, mTickRadius) * (2.0f + textLine);
        initStrokePaint();
...
//        initTickXAndTextXLocation();
    ...
    private int getMaxTextLine() {
        // checking how many line of thicktext,
        int textLine = 1;
        if (mTickTextsArr != null) {
            for (String tx : mTickTextsArr) {
                String[] x = tx.split("\n");
                if (x.length > textLine) {
                    textLine = x.length;
                }
            }
        }
    ...
    private void initTrackLocation() {
        ...
        if (mR2L) {
            ...
            if (hasAboveText()) {
                mBackgroundTrack.top = mPaddingTop + mThumbTouchRadius + (mDefaultTickTextsHeight * textLine) + SizeUtils.dp2px(mContext, 3);
            } else {
            ...
            if (hasAboveText()) {
                mProgressTrack.top = mPaddingTop + mThumbTouchRadius + (mDefaultTickTextsHeight * textLine) + SizeUtils.dp2px(mContext, 3);
            } else {
    ...
    private void drawTickTexts(Canvas canvas) {
        ...
        for (int i = 0; i < mTickTextsArr.length; i++) {
            ...
            String[] txt = mTickTextsArr[index] != null ? mTickTextsArr[index].split("\n") : new String[]{mTickTextsArr[index]};
            float txtSize = mTextPaint.getTextSize();
            float pos = 0;
            for (String tx : txt) {
/*
                if (i == 0) {
                    canvas.drawText(tx, mTextCenterX[i] + mTickTextsWidth[index] / 2.0f, mTickTextY + pos, mTextPaint);
                } else if (i == mTickTextsArr.length - 1) {
                    canvas.drawText(tx, mTextCenterX[i] - mTickTextsWidth[index] / 2.0f, mTickTextY + pos, mTextPaint);
                } else {
                    canvas.drawText(tx, mTextCenterX[i], mTickTextY + pos, mTextPaint);
                }
*/
                canvas.drawText(tx, mTextCenterX[i], mTickTextY + pos, mTextPaint);
                pos += (txtSize);
